...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go -cc clang-11 UretProbeBashHistory ./bpf/bashhistory.c -- -nostdinc -I../headers -O2
...
func main() {
	...
	objs := UretProbeBashHistoryObjects{}
	if err := LoadUretProbeBashHistoryObjects(&objs, nil); err != nil {
	...
	for {
		...
		log.Printf("%d:%s", event.UID, unix.ByteSliceToString(event.Line[:]))
	}
}
